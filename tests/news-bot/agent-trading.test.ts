...
describe("agent-trading candidates", () => {
  it("builds on-beat market-data candidates", () => {
    ...
    expect(candidates[0].content).toContain("For AIBTC agent traders");
    expect(candidates[0].content).toContain("0 STX");
    expect(candidates[0].content.length).toBeLessThanOrEqual(1000);
    expect(candidates.every((candidate) => candidate.content.includes("For AIBTC agent traders"))).toBe(true);
  });
